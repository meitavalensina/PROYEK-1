...
if ($status=='psa') {
	$result=mysqli_query($mysqli, "UPDATE pengajuan_surat SET status='Pembuatan Surat Diterima' WHERE id_ps='$id'");
	if ($result) {
	...
	$result=mysqli_query($mysqli, "UPDATE pengajuan_surat SET status='Pembuatan Surat Ditolak' WHERE id_ps='$id'");
	if ($result) {
	...
	$result=mysqli_query($mysqli, "UPDATE pengajuan_surat SET status='Surat Dibuat' WHERE id_ps='$id'");
	if ($result) {
	...
	$result=mysqli_query($mysqli, "UPDATE pengajuan_surat SET status='Surat Jadi' WHERE id_ps='$id'");
	if ($result) {
